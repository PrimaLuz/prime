...
class ElasticDeviceMesh:
    ...
    def __init__(
        ...
        self._logger.info(f'finished init global pg')

        # Initialize local process group
        dist.init_process_group(backend=backend)
        self._logger.info(f'finished dist.init_process_group')
        ...
        if self.enable:
            ...
            self._logger.info(f'finished optimize_ring_ranks')
            
            if self.live_recovery_rank_src is not None:
                self.live_recovery.ask_for_live_ckpt(self.live_recovery_rank_src)
                self._logger.info(f'finished live_recovery.ask_for_live_ckpt')
            self.global_pg.barrier().wait()
            self._logger.info(f'finished global_pg.barrier().wait()')
