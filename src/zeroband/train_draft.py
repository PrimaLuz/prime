...
def train(config: Config):
    ...
    logger.info(f"finished shard model using FSDP2")
    ...
    logger.info(f"need_live_recovery: {need_live_recovery}")
    while True:
        ...
        if config.diloco is not None:
            ...
            if not need_live_recovery:
                ...
                logger.info(f"live_recovery should_send_ckpt_to: {maybe_dest_rank}")
